...
def process_one_line(line: str) -> int:
    ...
    groups = 5 * [int(g) for g in groups.split(",")]
    lut = {}
    
    def dfs(springs_idx: int, group_idx: int, counter: int) -> int:
        if (springs_idx, group_idx) in lut:
            return lut[(springs_idx, group_idx)] + counter
        ...
        if len(springs) - springs_idx < sum(groups[group_idx:]) + len(groups) - group_idx - 1:
            return counter
        ...
        if springs[springs_idx] == "?":
            ...
            if (springs_idx + 1, group_idx) not in lut:
                lut[(springs_idx + 1, group_idx)] = counter_i - counter
        else:
            counter_i = 0
        ...
        if (springs_idx + groups[group_idx] + 1, group_idx + 1) not in lut:
            lut[(springs_idx + groups[group_idx] + 1, group_idx + 1)] = counter_j - counter
